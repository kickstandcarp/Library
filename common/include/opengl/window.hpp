...
enum class BlendFactor : GLenum { zero = GL_ZERO, one = GL_ONE, source_alpha = GL_SRC_ALPHA, one_minus_source_alpha = GL_ONE_MINUS_SRC_ALPHA }; 
...
class Window
{
    public:
		...
		void                                            add_frame_buffer(const std::string &name, const std::array<unsigned int, 2> &size, const std::vector<TextureFormat> &color_attachment_texture_formats, const bool has_depth_attachment);
    ...
    private:
		...
		std::map<std::string, FrameBuffer>::iterator    frame_buffer, target_frame_buffer;
