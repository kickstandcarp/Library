...
:   size(0.0f),
    ...
    source_color_attachment(0),
    intermediate_color_attachment(0),
    destination_color_attachment(0),
...
void GaussianFilter::apply(Window &window)
{
    float intermediate_frame_buffer_size_y = static_cast<float>(this->intermediate_frame_buffer_name == window.window_frame_buffer_name ? window.get_size()[1] : window.get_frame_buffer(this->intermediate_frame_buffer_name).get_size()[1]);
    float destination_frame_buffer_size_x = static_cast<float>(this->destination_frame_buffer_name == window.window_frame_buffer_name ? window.get_size()[0] : window.get_frame_buffer(this->destination_frame_buffer_name).get_size()[0]);

    window.set_use_blending(false);

    window.set_target_frame_buffer(this->intermediate_frame_buffer_name, {this->intermediate_color_attachment});
    window.clear();
	...
	window.get_frame_buffer(this->source_frame_buffer_name).get_color_texture(this->source_color_attachment).set_texture_unit(0);
    window.get_shader(this->shader_1_name).set_uniform("color_sampler", 0);
    ...
    window.set_target_frame_buffer(this->destination_frame_buffer_name, {this->destination_color_attachment});
    ...
    window.get_frame_buffer(this->intermediate_frame_buffer_name).get_color_texture(this->intermediate_color_attachment).set_texture_unit(0);
    window.get_shader(this->shader_2_name).set_uniform("intermediate_color_sampler", 0);
    window.get_frame_buffer(this->source_frame_buffer_name).get_color_texture(this->source_color_attachment).set_texture_unit(1);
    window.get_shader(this->shader_2_name).set_uniform("source_color_sampler", 1);
    window.get_vertex_array(this->vertex_array_name).draw();
}
