...
from parametric import CurveInterpolation, CurveVertexFloat, SegmentCurveFloat, CurveVertexVec2, SegmentCurveVec2
...
#     vertex_curve_vertices += [CurveVertexVec2(0.5*(vec2(vertex) - vec2(0.333, 0.5)), t + t0) for vertex, t in vertex_curves[character]]
#     excitation_curve_vertices += [CurveVertexFloat(1.0 if i % 2 == 0 else 0.0, t + t0) for i, (vertex, t) in enumerate(vertex_curves[character])]
#     t0 = vertex_curve_vertices[-1].t + 1.0
# vector_display_beam = VectorDisplayBeam(SegmentCurveVec2(vertex_curve_vertices, CurveInterpolation.linear), SegmentCurveFloat(excitation_curve_vertices, CurveInterpolation.floor), vec3(1.0), beam_radius, 5.0)
...
vertex_curve_vertices = [CurveVertexVec2(vec2(vertex), t) for vertex, t in data['vertex_curve']]
excitation_curve_vertices = [CurveVertexFloat(vertex, t) for vertex, t in data['excitation_curve']]
