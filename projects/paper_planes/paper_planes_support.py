...
from opengl import Window, BlendFactor, ShaderType, DrawMode, TextureFormat, TextureInterpolation, TextureWrap
from event import DeviceType
from geometry import CoordinateTransform, translation, rotation, scaling
from physics import OscillatorKineticsVec2, PaperKinetics
...
def initialize_window(size):
    ...
    window.get_vertex_array('arrow').add_element_buffer(sum([(0, 1), (1, 2), (1, 3)], ()))
    window.get_vertex_array('arrow').add_buffer('vertex_positions', [vec3(0.0, 0.0, 0.0), vec3(0.0, 1.0, 0.0), vec3(0.15, 0.75, 0.0), vec3(-0.15, 0.75, 0.0)])

    square_aspect_ratio = 0.1
    window.add_vertex_array('square', DrawMode.triangles)
    window.get_vertex_array('square').add_element_buffer(sum([(0, 1, 3), (1, 2, 3), (4, 5, 7), (5, 6, 7), (4, 5, 0), (5, 1, 0), (5, 6, 2), (6, 2, 1), (6, 7, 3), (7, 3, 2), (7, 4, 0), (4, 0, 3)], ()))
    window.get_vertex_array('square').add_buffer('vertex_positions', [vec3(-1.0, -square_aspect_ratio, -1.0), vec3(1.0, -square_aspect_ratio, -1.0), vec3(1.0, -square_aspect_ratio, 1.0), vec3(-1.0, -square_aspect_ratio, 1.0), vec3(-1.0, square_aspect_ratio, -1.0), vec3(1.0, square_aspect_ratio, -1.0), vec3(1.0, square_aspect_ratio, 1.0), vec3(-1.0, square_aspect_ratio, 1.0)])
    # window.get_vertex_array('square').add_buffer('vertex_texture_positions', [vec2(0.0, 0.0), vec2(1.0, 0.0), vec2(1.0, 1.0), vec2(0.0, 1.0)])

    # window.add_texture('plaid', image, TextureInterpolation.linear, TextureWrap.repeat)

    window.add_frame_buffer('draw', size, [TextureFormat.rgba], True)
...
class Player(object):
    def __init__(self, size, color, position):
        ...
        self.tilt_kinetics = OscillatorKineticsVec2(vec2(0.0), vec2(0.0), vec2(2.0), vec2(0.2))
    ...
    def update(self, event_handler, clock):
        ...
        self.coordinate_transform.rotate(quat(vec3(1.0, 0.0, 0.0), clock.elapsed_time*velocity_direction.y))
    ...
    def draw(self, window, camera):
        window.get_shader(self.arrow_shader_name).set_attribute('vertex_position', window.get_vertex_array(self.arrow_vertex_array_name), 'vertex_positions')

        window.get_shader(self.arrow_shader_name).set_uniform('transform_matrix', camera.transform_matrix*self.normal_arrow_coordinate_transform.global_matrix)
        window.get_shader(self.arrow_shader_name).set_uniform('color', self.normal_arrow_color)
        window.get_vertex_array(self.arrow_vertex_array_name).draw()

        window.get_shader(self.arrow_shader_name).set_uniform('transform_matrix', camera.transform_matrix*self.velocity_arrow_coordinate_transform.global_matrix)
        window.get_shader(self.arrow_shader_name).set_uniform('color', self.velocity_arrow_color)
        window.get_vertex_array(self.arrow_vertex_array_name).draw()
...
class Square(object):
    def __init__(self, size, color, position, orientation, velocity, angular_velocity):
        ...
        self.kinetics = PaperKinetics(position, orientation, velocity, angular_velocity, perpendicular_friction=10.0, parallel_friction=0.1, fluid_density=0.1, paper_density=1.0, size=vec3(size[0], 0.0, size[1]))
        self.kinetics.external_acceleration = vec3(0.0, -9.8, 0.0)
        self.kinetics.min_step = 1.0e-3
        self.kinetics.relative_step_tolerance = 1.0e-3, 1.0e-3

    def update(self, clock):
        self.kinetics.step(clock)
        self.coordinate_transform.translation = self.kinetics.position
        self.coordinate_transform.rotation = self.kinetics.orientation

    def draw(self, window, camera):
        window.get_shader(self.shader_name).set_attribute('vertex_position', window.get_vertex_array(self.vertex_array_name), 'vertex_positions')
        # window.get_shader(self.shader_name).set_attribute('vertex_texture_position', window.get_vertex_array(self.vertex_array_name), 'vertex_texture_positions')

        window.get_shader(self.shader_name).set_uniform('transform_matrix', camera.transform_matrix*self.coordinate_transform.global_matrix)
        window.get_shader(self.shader_name).set_uniform('color', self.color)
        # window.get_shader(self.shader_name).set_uniform('texture_sampler', 0)
        # window.textures('').set_texture_unit(0)

        window.get_vertex_array(self.vertex_array_name).draw()
